...
def grabdata(loc,list,simname='test'):
    ...
    for file in files:
        ...
        while keepgoing:
            ...
            if 'SNID:' in sn[i]:
                snid = sn[i].split()[1]
            if 'SIM_NON1a:' in sn[i]:
                sntype = sn[i].split()[1]
                #keepgoing = False
            if 'HOSTGAL_PHOTOZ:' in sn[i]:
        ...
        sndata = dt.read(file,startline,startline+1)
        flux = sndata['FLUXCAL']
    ...
    dsntypes[sntypes != 0] = np.ones(len(sntypes[sntypes != 0]))
    dsntypes[sntypes == 0] = np.zeros(len(sntypes[sntypes == 0]))
...
if __name__ == '__main__':
    ...
    hostloc = '/project/rkessler/SN/SNDATA_ROOT/SIM/DB_smearG10+CCx1'
    hostlist= '/project/rkessler/SN/SNDATA_ROOT/SIM/DB_smearG10+CCx1/DB_smearG10+CCx1.LIST'
    
    grabdata(hostloc,hostlist,simname='smearG10+CCx1')
